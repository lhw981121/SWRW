...
function SelectPage(pageNo){
	$('#approvedCompanyData').append('<td colspan="7"><h3 style="text-align:center">'+(language=='zh_CN'?'数据加载中。。。':'Data loading...')+'</h3></td>');
	page = pageNo;
	$.ajax({
		type:"post",
		...
		success:function(result) {
			...
			/* 渲染数据 */
			var company = r.company;
			$('#approvedCompanyData').html('');
			if(company.length==0){$('#approvedCompanyData').html('<td colspan="7"><h3 style="text-align:center">'+(language=='zh_CN'?'没有数据':'No Data')+'</h3></td>');}
			$.each(company, function (index, com) {
				$('#approvedCompanyData').append(
				'<tr>'
				...
				+'<td style="vertical-align:middle;">'+getCompanyState(com.company_state)+'</td>'
				+'</tr>');
			});
			/* 渲染页码 */
