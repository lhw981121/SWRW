...
function SelectPage(pageNo){
	$('#approvedJobData').append('<td colspan="5"><h3 style="text-align:center">'+(language=='zh_CN'?'数据加载中。。。':'Data loading...')+'</h3></td>');
	page = pageNo;
	$.ajax({
		type:"post",
		...
		success:function(result) {
			...
			/* 渲染数据 */
			var jobs = r.jobs;
			$('#approvedJobData').html('');
			if(jobs.length==0){$('#approvedJobData').html('<td colspan="5"><h3 style="text-align:center">'+(language=='zh_CN'?'没有数据':'No Data')+'</h3></td>');}
			$.each(jobs, function (index, job) {
				$('#approvedJobData').append(
				'<tr>'
				...
				+'<td style="vertical-align:middle;">'+getCompanyState(job.job_state)+'</td>'
				+'</tr>');
			});
			/* 渲染页码 */
