...
public class CompanyDao {
	...
	* @return 企业对象
	*/
	public Company queryUserBySingleData(String str,String value){
		Connection conn = DBUtil.getConnection();
		PreparedStatement pstmt = null;
	    ResultSet rs = null;
	    Company obj = null;
	...
	*/
	...
	/**
	* 获取分页企业信息数据总数量
	* @param company_state 企业招聘状态：-2(审核未通过)、-1(审核中)、0(不可用)、1(招聘中)、2(已暂停)、3(已结束)
	...
	*/
	public int getPageDataCompanyCount(int company_state,String company_area,String company_size,String company_type,String queryStr,int deleted) {
	    ...
	    try{
    		...
    		+ "and deleted=?";
	        pstmt = conn.prepareStatement(sqlQuery);
	        pstmt.setInt(index++, company_state);
